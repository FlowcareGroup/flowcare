...
import { editClinic, getClinicById } from "@/services/api/clinicsServices";
...
export default function editClinicPage() {
  ...
  useEffect(() => {
    if (session?.accessToken) {
      ClinicById(id, (session as any).accessToken);
    }
  }, [id, reset, session]);

  const ClinicById = async (id: number, token: string) => {
    try {
      const response = await getClinicById(id, token);
  ...
  const onSubmit = async (data: ClinicEditSchema) => {
    try {
      const response = await editClinic(data, id, (session as any).accessToken);
